...
Eg: Find the green function of the operator $L=D^{2}+2D-15$
Solution: 
The characteristic polynomial is $p(s)=s^{2}+2s-15=(s-3)(s+5)$
Thus $G(s)=\frac{1}{(s-3)(s+5)}=\frac{1/8}{s-3}-\frac{1/8}{s+5}$
So $g(t)=\mathcal{L}^{-1}[G](t)=\frac{1}{8}\mathcal{L}^{-1}[\frac{1}{s-3}]-\frac{1}{8}\mathcal{L}^{-1}[\frac{1}{s+5}]=\frac{e^{3t}-e^{-5t}}{8}$